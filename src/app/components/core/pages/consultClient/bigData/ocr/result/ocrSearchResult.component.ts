...
export class ocrSearchResultComponent implements OnInit, OnDestroy {
  ...
  public createTabelWithDataByType(type:string, response:Root) {
     if(type == 'pf'){
      ...
      response.Result[0].LawsuitsDistributionData ? this.processInformationCnjSubjectDistribution = [this.ocrService.filterPocessInformation(response.Result[0].LawsuitsDistributionData.CnjSubjectDistribution)] : this.processInformationCnjSubjectDistribution = []
      response.Result[0].LawsuitsDistributionData ? this.processInformationCnjProcedureTypeDistribution = [this.ocrService.filterPocessInformation(response.Result[0].LawsuitsDistributionData.CnjProcedureTypeDistribution)] : this.processInformationCnjProcedureTypeDistribution = []
      response.Result[0].LawsuitsDistributionData ? this.processInformationCourtLevelDistribution = [this.ocrService.filterPocessInformation(response.Result[0].LawsuitsDistributionData.CourtLevelDistribution)] : this.processInformationCourtLevelDistribution = []
      response.Result[0].LawsuitsDistributionData ? this.processInformationCourtNameDistribution = [this.ocrService.filterPocessInformation(response.Result[0].LawsuitsDistributionData.CourtNameDistribution)] : this.processInformationCourtNameDistribution = []
      response.Result[0].LawsuitsDistributionData ? this.processInformationCourtTypeDistribution = [this.ocrService.filterPocessInformation(response.Result[0].LawsuitsDistributionData.CourtTypeDistribution)] : this.processInformationCourtTypeDistribution = []
      response.Result[0].LawsuitsDistributionData ? this.processInformationPartyTypeDistribution = [this.ocrService.filterPocessInformation(response.Result[0].LawsuitsDistributionData.PartyTypeDistribution)] : this.processInformationPartyTypeDistribution = []
      response.Result[0].LawsuitsDistributionData ? this.processInformationStateDistribution = [this.ocrService.filterPocessInformation(response.Result[0].LawsuitsDistributionData.StateDistribution)] : this.processInformationStateDistribution = []
      response.Result[0].LawsuitsDistributionData ? this.processInformationStatusDistribution = [this.ocrService.filterPocessInformation(response.Result[0].LawsuitsDistributionData.StatusDistribution)] : this.processInformationStatusDistribution = []
      response.Result[0].LawsuitsDistributionData ? this.processInformationTypeDistribution = [this.ocrService.filterPocessInformation(response.Result[0].LawsuitsDistributionData.TypeDistribution)] : this.processInformationTypeDistribution = []
